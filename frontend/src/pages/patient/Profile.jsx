...
export default function Profile() {
  ...
  const fetchProfile = async () => {
    try {
      setLoading(true);
      // Fix: Change from /api/patients/profile to /api/patient/profile
  ...
  const handleSave = async () => {
    ...
    try {
      // Fix: Change from /api/patients/profile to /api/patient/profile
      await axiosInstance.put('/api/patient/profile', form);
      ...
      <div style={{ background: '#f0f7f3', flex: 1, padding: '24px 32px', marginTop: 132 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1.5fr', gap: 20 }}>
          ...
          <div style={{ background: '#fff', borderRadius: 8, border: '0.5px solid #e2e8f0', padding: 24, textAlign: 'center' }}>
            ...
            <div style={{ fontSize: 16, fontWeight: 700, color: '#1e293b' }}>{user?.name || 'User'}</div>
            <div style={{ fontSize: 12, color: '#64748b', marginTop: 4 }}>{profile?.email || user?.email}</div>
            ...
            <div style={{ marginTop: 20, display: 'flex', flexDirection: 'column', gap: 10 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, padding: '8px 0', borderBottom: '0.5px solid #f1f5f9' }}>
                ...
                <span style={{ fontWeight: 700, color: '#e74c3c' }}>{form.blood_group || profile?.blood_group || '—'}</span>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, padding: '8px 0', borderBottom: '0.5px solid #f1f5f9' }}>
                <span style={{ color: '#64748b' }}>Phone</span>
                <span style={{ fontWeight: 600 }}>{form.phone || profile?.phone || '—'}</span>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, padding: '8px 0' }}>
                <span style={{ color: '#64748b' }}>Address</span>
                <span style={{ fontWeight: 600, textAlign: 'right', maxWidth: 120 }}>{form.address || profile?.address || '—'}</span>
              </div>
            </div>
          ...
          <div style={{ background: '#fff', borderRadius: 8, border: '0.5px solid #e2e8f0', padding: 24 }}>
            ...
            {success && (
              <div style={{ background: '#e8f5ee', color: '#1a6b3a', padding: '10px 14px', borderRadius: 6, fontSize: 13, marginBottom: 14 }}>
                ✅ {success}
              </div>
            )}
            {error && (
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
              ...
              <div>
                ...
                <input value={profile?.email || user?.email || ''} disabled style={{ ...inputStyle, background: '#f8fafc', color: '#94a3b8' }} />
              </div>
              <div>
            ...
            {editing && (
              <div style={{ display: 'flex', gap: 10, marginTop: 20 }}>
                ...
                <button onClick={() => { setEditing(false); setError(''); setSuccess(''); fetchProfile(); }} style={{ flex: 1, padding: '10px', background: '#f1f5f9', color: '#64748b', border: 'none', borderRadius: 6, fontSize: 13, cursor: 'pointer', fontWeight: 600 }}>
                  Cancel
                </button>
              </div>
            )}
